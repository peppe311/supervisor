        ...
        for out in outputs {
            match out {
                ...
                ActiveStageOutput::GraphicsUpdate(region) => {
                    ...
                    let right = region.right.min(image.width().saturating_sub(1));
                    let bottom = region.bottom.min(image.height().saturating_sub(1));
                    if region.left > right || region.top > bottom {
                        continue;
                    }
                    let width = NonZeroU16::new(right - region.left + 1)
                        .ok_or_else(|| ironrdp_session::general_err!("region width is zero"))?;
                    let height = NonZeroU16::new(bottom - region.top + 1)
                        .ok_or_else(|| ironrdp_session::general_err!("region height is zero"))?;
                    let source = image.data();
                    let stride = usize::from(image.width()) * 4;
                    let row_bytes = usize::from(width.get()) * 4;
                    let mut buffer = Vec::with_capacity(usize::from(width.get()) * usize::from(height.get()));
                    for row in region.top..=bottom {
                        let start = usize::from(row) * stride + usize::from(region.left) * 4;
                        let end = start + row_bytes;
                        buffer.extend(source[start..end].chunks_exact(4).map(|pixel| {
                            let r = pixel[0];
                            let g = pixel[1];
                            let b = pixel[2];
